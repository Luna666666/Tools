...
//iPhone 7（9.2）以上在震动--短震动触感
@property (nonatomic, assign)float deviceModel;
...
- (void)prepare
{
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 10.0) {
        ...
        NSInteger length = strlen(machine);
        if (length >= 3) {
            self.deviceModel = [[NSString stringWithFormat:@"%d.%d",machine[length - 3] -'0' ,machine[length-1]-'0'] floatValue];
        }
        free(machine);
    }
}
...
- (void)setState:(MJRefreshState)state
{
    ...
    if (state == MJRefreshStatePulling && ([[UIDevice currentDevice].systemVersion floatValue] >= 10.0) && self.deviceModel > 9) {
