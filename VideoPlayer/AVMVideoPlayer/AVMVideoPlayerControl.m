...
@interface AVMVideoPlayerControl () <UIGestureRecognizerDelegate>
...
@implementation AVMVideoPlayerControl

//优化点击区域
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    UIView *hitView = [super hitTest:point withEvent:event];
    CGPoint topPoint = [self convertPoint:point toView:self.topImageView];
    if ((!self.topImageView.hidden && self.topImageView.alpha != 0 )&& CGRectContainsPoint(self.backBtn.frame, topPoint)) {
        return self.backBtn;
    }
    CGPoint bottomPoint = [self convertPoint:point toView:self.bottomImageView];
    if (self.showing && self.ADImageView.hidden) {
        if (CGRectContainsPoint(self.startBtn.frame, bottomPoint)) {
            return self.startBtn;
        }
        if (CGRectContainsPoint(self.fullScreenBtn.frame, bottomPoint)) {
            return self.fullScreenBtn;
        }
    }
    CGPoint ADPoint = [self convertPoint:point toView:self.ADImageView];
    if ((!self.ADImageView.hidden && self.ADImageView.alpha != 0)&& CGRectContainsPoint(self.ADCloseBtn.frame, ADPoint)) {
        return self.ADCloseBtn;
    }
    return hitView;
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        [self.placeholderImageView addSubview:self.HDLabel];
        ...
        [self.ADImageView addSubview:self.ADCloseBtn];
...
- (void)makeSubViewsConstraints {
    ...
    [self.HDLabel mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.videoLengthLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.trailing.equalTo(self).offset(-7);
    ...
    [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(self.backBtn.mas_centerY);
        make.trailing.equalTo(self.resolutionBtn.mas_leading).offset(30);
    }];
    ...
    [self.startBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.bottomImageView.mas_bottom).offset(2);
    ...
    [self.totalTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.trailing.equalTo(self.fullScreenBtn.mas_leading).offset(7);
        make.centerY.equalTo(self.startBtn.mas_centerY);
        make.width.mas_equalTo(43);
    }];
    ...
    [self.ADImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(kScreenWidth/kAVMADScale);
    }];
    [self.ADCloseBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.height.mas_equalTo(30);
        make.top.mas_offset(0);
        make.trailing.mas_offset(2);
    }];
}
...
#pragma mark -优化左滑手势和slider的手势问题
- (void)panRecognizer1:(UIPanGestureRecognizer *)sender {
    sender.enabled = NO;
    sender.enabled = YES;
//    return;
//    CGPoint point = [sender translationInView:self.videoSlider];
//    
//    static CGFloat beginValue = 0;
//    CGFloat width = self.videoSlider.frame.size.width;
//    switch (sender.state) {
//        case UIGestureRecognizerStateBegan:
//        {
//            CGRect rect = [self thumbRect];
//            if (point.x <= rect.origin.x + rect.size.width && point.x >= rect.origin.x) {
//                beginValue = self.videoSlider.value;
...
//                return;
//            }
//            self.videoSlider.value = point.x/width + beginValue;
...
- (void)fullScreenBtnClick:(UIButton *)sender {
    ...
    if (!self.playerView.isPauseByUser) {
        if (!self.fullScreen) {
            if ((CMTimeGetSeconds(self.playerView.playerItem.duration) -CMTimeGetSeconds(self.playerView.playerItem.currentTime)) <= 3.f) {
                return;
            }
        }
    }
...
- (void)openADTapGeClick:(UITapGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateEnded) {
//        [self.playerView pause];
        if (self.isFull) {
            [self fullScreenBtnClick:nil];
            //延时长一点，不然屏幕旋转会有问题
...
- (void)openADWeb {
    [AVMTool openURL:[NSURL URLWithString:self.ADWebUrl] completionHandler:^(BOOL success) {
        if (!success) {
        }
    }];
}

- (void)closeADBtnClick:(UIButton *)sender {
    //关闭广告
...
//    self.repeatBtn.hidden = NO;
    ...
    self.videoLengthLabel.hidden = !self.ADImageView.hidden;
...
- (void)onDeviceOrientationChange {
    ...
    if (!self.isShrink && !self.isPlayEnd && !self.showing) {
        // 显示、隐藏控制层
        if (self.playerView.state != ZFPlayerStatePlaying || self.playerView.state != ZFPlayerStatePause) {
            return;
        }
...
- (void)setOrientationLandscapeConstraint {
    ...
    self.lockBtn.hidden         = !self.isFullScreen;
    self.fullScreenBtn.selected = self.isFullScreen;
    [self.backBtn setImage:kUIImage(@"app_icon_back") forState:UIControlStateNormal];
    [self.backBtn setTitle:@"返回" forState:UIControlStateNormal];
    self.backBtn.titleLabel.font = kSystemFont(16);
    self.backBtn.titleEdgeInsets = UIEdgeInsetsMake(0, -6, 0, 0);
...
//    }];
    ...
    if (self.isCellVideo) {
        ...
        self.backBtn.imageEdgeInsets = UIEdgeInsetsZero;
    }
}
...
- (void)showControlView {
    ...
    if (self.lockBtn.isSelected) {
        ...
        self.topImageView.alpha    = self.isFullScreen?1:0;
...
- (void)hideControlView {
    ...
    self.topImageView.alpha       = self.isFullScreen?self.playeEnd:0;
    self.bottomImageView.alpha    = 0;
 ...
 */
- (void)showAD {
    if (!self.ADImageView.hidden) return;
    if (!self.ADWebUrl) {
        self.ADImageView.hidden = YES;
        return;
    }
    if (self.currentPlayTime > 2 && !self.isDragged) {
...
- (UILabel *)HDLabel {
    if (!_HDLabel) {
        ...
        _HDLabel.font = [UIFont systemFontOfSize:14.0];
        _HDLabel.textAlignment = NSTextAlignmentCenter;
        _HDLabel.text = @"HD";
        _HDLabel.backgroundColor = [UIColor blackColor];
        _HDLabel.hidden = YES;
    }
    return _HDLabel;
}
...
- (UILabel *)videoLengthLabel {
    if (!_videoLengthLabel) {
        ...
        _videoLengthLabel.font = [UIFont systemFontOfSize:12.0];
        _videoLengthLabel.textAlignment = NSTextAlignmentCenter;
        _videoLengthLabel.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.6];
...
- (UIButton *)backBtn {
    if (!_backBtn) {
        ...
        [_backBtn setImage:kUIImage(@"app_icon_back") forState:UIControlStateNormal];
...
- (UIButton *)startBtn {
    if (!_startBtn) {
        ...
        [_startBtn setImage:kUIImage(@"app_icon_play") forState:UIControlStateNormal];
        [_startBtn setImage:kUIImage(@"app_icon_stop") forState:UIControlStateSelected];
...
- (UILabel *)currentTimeLabel {
    if (!_currentTimeLabel) {
        ...
        _currentTimeLabel.textColor     = kTextColor_d8d8d8;
...
- (ASValueTrackingSlider *)videoSlider {
    if (!_videoSlider) {
        ...
        UIImage *thumbImage = [kUIImage(@"timeline_icon_button") imageByResizeToSize:CGSizeMake(12, 12)];
        UIImage *normalThumbImage = [thumbImage imageByRoundCornerRadius:6 borderWidth:1 borderColor:kThemeColor];
        
        UIImage *thumbImageH = [kUIImage(@"timeline_icon_button") imageByResizeToSize:CGSizeMake(16, 16)];
        UIImage *hThumbImage = [thumbImageH imageByRoundCornerRadius:8 borderWidth:2 borderColor:kThemeColor];
        [_videoSlider setThumbImage:normalThumbImage forState:UIControlStateNormal];
        [_videoSlider setThumbImage:hThumbImage forState:UIControlStateHighlighted];
        
//        [_videoSlider setMinimumTrackImage:kUIImage(@"timeline_pic_noplay") forState:UIControlStateNormal];
//        [_videoSlider setMaximumTrackImage:kUIImage(@"timeline_pic_play") forState:UIControlStateNormal];
        
        _videoSlider.maximumValue          = 1;
        _videoSlider.minimumTrackTintColor = kThemeColor;
        ...
        //解决左滑手势问题
        UIPanGestureRecognizer *panRecognizer1 = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(panRecognizer1:)];
        [panRecognizer1 setMaximumNumberOfTouches:1];
        [panRecognizer1 setDelaysTouchesBegan:YES];
        [_videoSlider addGestureRecognizer:panRecognizer1];
        ...
        UIPanGestureRecognizer *panRecognizer2 = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(panRecognizer:)];
        [panRecognizer2 setMaximumNumberOfTouches:1];
        [panRecognizer2 setDelaysTouchesBegan:YES];
        [self.bottomImageView addGestureRecognizer:panRecognizer2];
        
        [panRecognizer requireGestureRecognizerToFail:panRecognizer1];
        
        [panRecognizer2 requireGestureRecognizerToFail:panRecognizer1];
    }
    return _videoSlider;
}
...
- (UILabel *)totalTimeLabel {
    if (!_totalTimeLabel) {
        ...
        _totalTimeLabel.textColor     = kTextColor_d8d8d8;
...
- (UIButton *)fullScreenBtn {
    if (!_fullScreenBtn) {
        ...
        [_fullScreenBtn setImage:kUIImage(@"app_icon_amplify") forState:UIControlStateNormal];
        [_fullScreenBtn setImage:kUIImage(@"app_icon_shrink") forState:UIControlStateSelected];
...
- (UIButton *)playeBtn {
    if (!_playeBtn) {
        ...
        [_playeBtn setImage:kUIImage(@"videodetails_icon_play1") forState:UIControlStateNormal];
...
- (UIProgressView *)fastProgressView {
    if (!_fastProgressView) {
        ...
        _fastProgressView.progressTintColor = kThemeColor;
...
- (UIImageView *)ADImageView {
    if (!_ADImageView) {
        ...
        _ADImageView.backgroundColor = [UIColor blackColor];
        
        UITapGestureRecognizer *tapGe = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(openADTapGeClick:)];
        [_ADImageView addGestureRecognizer:tapGe];
    }
    return _ADImageView;
}
...
- (UIButton *)ADCloseBtn {
    if (!_ADCloseBtn) {
        _ADCloseBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_ADCloseBtn setImage:kUIImage(@"ad_icon_close_normal") forState:UIControlStateNormal];
        [_ADCloseBtn setImage:kUIImage(@"ad_icon_close_pressed") forState:UIControlStateHighlighted];
        [_ADCloseBtn addTarget:self action:@selector(closeADBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _ADCloseBtn;
}

#pragma mark - UIGestureRecognizerDelegate
#pragma mark -优化左滑手势冲突
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRequireFailureOfGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    
    if ([otherGestureRecognizer isEqual:[(AVMNavigationController *)[AVMTool rootViewController] fd_fullscreenPopGestureRecognizer]]) {
        return NO;
    }
    if (!self.ADImageView.hidden) {
        CGPoint point = [gestureRecognizer locationInView:self];
        if (CGRectContainsPoint(self.ADImageView.frame, point)) {
            return NO;
        }
    }
    return YES;
...
//    }
    ...
    if ([touch.view isEqual:self.backBtn] || CGRectContainsPoint(self.backBtn.frame, point)) {
        return NO;
    }
    return YES;
}
...
- (void)showHD:(BOOL)hd {
    self.HDLabel.hidden = !hd;
}

- (void)showVideoTimeLength:(NSString *)length {
    self.videoLengthLabel.text = length?length:@"00:00";
    self.videoLengthLabel.hidden = NO;
    self.totalTimeLabel.text = length?length:@"00:00";
}
...
- (void)zf_playerModel:(ZFPlayerModel *)playerModel {
    ...
    if (playerModel.placeholderImageURLString) {
        [self.placeholderImageView sd_setImageWithURL:[NSURL URLWithString:playerModel.placeholderImageURLString] placeholderImage:kFilmCoverDeaflutImage()];
    } else {
    ...
    //广告
    if (playerModel.ADCoverImageUrl) {
        [self.ADImageView sd_setImageWithURL:[NSURL URLWithString:playerModel.ADCoverImageUrl]];
        self.ADWebUrl = playerModel.ADWebUrl;
    }else {
        self.ADImageView.hidden = YES;
        self.ADWebUrl = nil;
    }
